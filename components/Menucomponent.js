  ...
  return (
    ...
    <Pressable style={{margin:10, flexDirection:'row',  justifyContent:'space-between'}}>
        <View>
          ...
          <Text style={{width:180, marginTop:8, fontSize:16, color:'grey'}}>{food.description.length> 30 ? food.description.substr(0,35) + "..." : food.description}</Text>
        </View>
      <Pressable style={{marginRight:10,}}>
        <Image style={{width:120, height:120, borderRadius:8,}} source={{uri:food.image}}/>
        <Pressable style={{position:'absolute', top:90, left:20, flexDirection:'row', paddingHorizontal:25,paddingVertical:10,alignItems:'center', backgroundColor:'white', borderRadius:5,}}>
          <Text style={{fontSize:18, fontWeight:'800', color:"#018749"}}>ADD</Text>
        </Pressable>
      </Pressable>
